...
<head>
    ...
    <style>
        ...
        .checkbox-group {{
            max-height: 120px; overflow-y: auto; border: 1px solid #aaa;
        ...
        .btn-small {{
            font-size: 11px; padding: 2px 5px; margin: 2px;
            background: #e0e0e0; border: 1px solid #aaa; cursor: pointer;
        }}
        .clear-btn {{
...
<body>
    ...
    <div class="filter-container">
        ...
        <div class="filter-group">
            ...
            <div id="filterCountContainer"></div>
        </div>
        <div class="filter-group">
            <label>排除维度数量</label>
        ...
        <div class="filter-group">
            ...
            <div id="filterMetricsContainer"></div>
        </div>
...
"""
    ...
    <script>
        ...
        // 渲染等于模式的 checkbox 列表
        function renderCheckboxGroup(containerId, values, selectedValues) {
            ...
            values.forEach(v => {
                ...
                cb.type = 'checkbox';
                cb.value = v;
                if (selectedValues && selectedValues.includes(String(v))) cb.checked = true;
            ...
            selectAll.onclick = (e) => {
                e.preventDefault();
                let cbs = groupDiv.querySelectorAll('input[type="checkbox"]');
                cbs.forEach(cb => cb.checked = true);
                filterTable();
            };
            let clearAll = document.createElement('button');
            clearAll.textContent = '清空';
            clearAll.className = 'btn-small';
            clearAll.onclick = (e) => {
                e.preventDefault();
                let cbs = groupDiv.querySelectorAll('input[type="checkbox"]');
            ...
            btnDiv.appendChild(selectAll);
            btnDiv.appendChild(clearAll);
            container.appendChild(groupDiv);
            container.appendChild(btnDiv);
        }

        // 渲染大于/大于等于模式的单选下拉框（含“全部”）
        function renderRadioSelect(containerId, values, selectedValue) {
            let container = document.getElementById(containerId);
            container.innerHTML = '';
            ...
            values.forEach(v => {
                ...
                opt.value = v;
                opt.text = v;
                if (selectedValue !== undefined && String(selectedValue) === String(v)) {
                    opt.selected = true;
                }
        ...
        // 获取当前选中的值（用于筛选）
        function getSelectedValues(containerId, op) {
            let container = document.getElementById(containerId);
            if (op === '=') {
                let cbs = container.querySelectorAll('.checkbox-group input[type="checkbox"]:checked');
        ...
        // 重置指定列的控件（切换操作符时调用）
        function resetNumberFilter(attr, containerId, opSelectId) {
            ...
            if (op === '=') {
                ...
                renderRadioSelect(containerId, values, '');
            }
            filterTable();
        }
        ...
        function initNumberFilters() {
            ...
            if (countOp === '=') renderCheckboxGroup('filterCountContainer', countValues, []);
            else renderRadioSelect('filterCountContainer', countValues, '');

            if (dimsOp === '=') renderCheckboxGroup('filterDimsContainer', dimsValues, []);
            else renderRadioSelect('filterDimsContainer', dimsValues, '');

            if (metricsOp === '=') renderCheckboxGroup('filterMetricsContainer', metricsValues, []);
            else renderRadioSelect('filterMetricsContainer', metricsValues, '');
        }
        ...
        function filterTable() {
            ...
            let countOp = document.getElementById('filterCountOp').value;
            let countSelected = getSelectedValues('filterCountContainer', countOp);
            let dimsOp = document.getElementById('filterDimsOp').value;
            let dimsSelected = getSelectedValues('filterDimsContainer', dimsOp);
            let metricsOp = document.getElementById('filterMetricsOp').value;
            let metricsSelected = getSelectedValues('filterMetricsContainer', metricsOp);
        ...
        window.addEventListener('DOMContentLoaded', () => {
            ...
            // 操作符变化时重置控件
            document.getElementById('filterCountOp').addEventListener('change', () => resetNumberFilter('count', 'filterCountContainer', 'filterCountOp'));
            document.getElementById('filterDimsOp').addEventListener('change', () => resetNumberFilter('dims', 'filterDimsContainer', 'filterDimsOp'));
            document.getElementById('filterMetricsOp').addEventListener('change', () => resetNumberFilter('metrics', 'filterMetricsContainer', 'filterMetricsOp'));
            
            // 监听 checkbox 变化和 select 变化（通过事件委托）
            document.getElementById('filterCountContainer').addEventListener('change', debouncedFilter);
            document.getElementById('filterDimsContainer').addEventListener('change', debouncedFilter);
            document.getElementById('filterMetricsContainer').addEventListener('change', debouncedFilter);
